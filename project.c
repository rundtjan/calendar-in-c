...
void try_delete_entry(char* command_buffer, CalEntry* db, int* db_size){
  int month, day, hour;
...
void create_entry_in_db(CalEntry* db, char* description, int month, int day, int hour, int* db_size){
  char* desc_ptr = (char*)malloc(strlen(description)+1);
  memcpy(desc_ptr, description, strlen(description)+1);
  CalEntry entry = {desc_ptr, month, day, hour, NULL};
  CalEntry* new_db_entry = (CalEntry*)malloc(sizeof(CalEntry));
...
void try_add_entry(char* command_buffer, CalEntry* db, int* db_size){
...
int main(void) {
  ...
  int db_size = 0;
  CalEntry* db = init_db(&db_size);
  if(db == NULL) return 0;
  //enter_test_entries(db, &db_size);
  while(1){
